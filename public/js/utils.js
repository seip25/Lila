...
class ResponsiveDataTable {
  ...
  renderDesktopTable() {
    ...
    const startIndex = (this.currentPage - 1) * this.options.rowsPerPage; const endIndex = startIndex + this.options.rowsPerPage; const paginatedData = this.filteredData.slice(startIndex, endIndex); const tbody = table.querySelector("tbody"); paginatedData.forEach((item) => {
      const row = document.createElement("tr"); this.options.columns.forEach((column) => {
        const td = document.createElement("td"); const value = item[column.key]; if (value && typeof value === "string" && /<[a-z][\s\S]*>/i.test(value)) { td.innerHTML = value; } else { td.textContent = value || "-"; }
        row.appendChild(td);
      }); if (this.options.edit || this.options.delete) {
        const td = document.createElement("td"); const actionsDiv = document.createElement("div"); actionsDiv.className = "mt flex justify-between gap-4"; if (this.options.edit) { const btn = document.createElement("button"); btn.className = "outline"; btn.textContent = lang() ? "Editar" : "Edit"; btn.onclick = (e) => this.handleAction("edit", e, item); actionsDiv.appendChild(btn); }
  ...
  renderMobileView() {
    const mobileView = this.container.querySelector(".datatable-mobile"); mobileView.innerHTML = ""; const startIndex = (this.currentPage - 1) * this.options.rowsPerPage; const endIndex = startIndex + this.options.rowsPerPage; const paginatedData = this.filteredData.slice(startIndex, endIndex); paginatedData.forEach((item) => {
      const card = document.createElement("article"); const summary = document.createElement("h3"); this.options.summaryFields.forEach((fieldKey) => { const value = item[fieldKey]; summary.innerHTML += `${value || "-"}<br>`; }); card.appendChild(summary); const details = document.createElement("dl"); this.options.columns.forEach((column) => {
        if (this.options.summaryFields.includes(column.key)) return; const dt = document.createElement("dt"); dt.textContent = this.options.headerTitles[column.key] || column.title || column.key; const dd = document.createElement("dd"); const cellValue = item[column.key]; if (cellValue && typeof cellValue === "string" && /<[a-z][\s\S]*>/i.test(cellValue)) { dd.innerHTML = cellValue; } else { dd.textContent = cellValue || "-"; }
        details.appendChild(dt); details.appendChild(dd);
      }); card.appendChild(details); if (this.options.edit || this.options.delete) {
        const actions = document.createElement("div"); actions.className = "mt flex justify-between gap-4"; if (this.options.edit) { const btn = document.createElement("button"); btn.className = "outline"; btn.textContent = lang() ? "Editar" : "Edit"; btn.onclick = (e) => this.handleAction("edit", e, item); actions.appendChild(btn); }
